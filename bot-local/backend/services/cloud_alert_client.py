...
class CloudAlertClientService:
    ...
    async def _handle_cloud_event(self, event):
        ...
        if etype == "donation_alert":
            ...
            if event.get("is_forwarded") and cat == "CHAT":
                return # Ignore chat logs echoed back from cloud
            # Ignore log categories natively generated by Local Pi
            if cat in ["DONATION", "APP_NOTIFICATION", "ALERT"]:
                return
            # Pass cloud logs to the local UI
            msg = event.get("message")
            auth = event.get("author")
            meta = event.get("meta")
            if cat and msg and hasattr(self.bot, "_log_ui"):
                asyncio.create_task(self.bot._log_ui(cat, msg, author=auth, meta=meta))
